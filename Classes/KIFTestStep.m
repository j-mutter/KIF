...
+ (id)stepToWaitForAbsenceOfViewWithAccessibilityLabel:(NSString *)label value:(NSString *)value traits:(UIAccessibilityTraits)traits;
{
    ...
    return [self stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        KIFTestWaitCondition([view isHidden] || (view.alpha == 0), error, @"Accessibility element with label \"%@\" is visible and not hidden.", label);
...
{
    return [self stepToTapViewWithAccessibilityLabelLike:label traits:UIAccessibilityTraitNone class:class];
}

+ (id)stepToTapViewWithAccessibilityLabelLike:(NSString *)label traits:(UIAccessibilityTraits)traits class:(Class)class
{
    return [self stepToTapViewWithAccessibilityLabelLike:label value:nil traits:traits class:class];
}

+ (id)stepToTapViewWithAccessibilityLabelLike:(NSString *)label value:(NSString *)value traits:(UIAccessibilityTraits)traits class:(Class)class
{
    NSString *description = nil;
    if (value.length) {
    ...
    return [self stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        UIAccessibilityElement *element = [self _accessibilityElementWithLabelLike:label accessibilityValue:value tappable:YES traits:traits class:class error:error];
        if (!element) {
            return KIFTestStepResultWait;
        }
        ...
        for (NSInteger componentIndex = 0; componentIndex < componentCount; componentIndex++) {
            ...
            for (NSInteger rowIndex = 0; rowIndex < rowCount; rowIndex++) {
                ...
                if ([rowTitle isEqual:title]) {
                    ...
                    KIFTestCondition([self tapView:pickerView error:error] != KIFTestStepResultFailure, error, @"Could not tap picker view");
...
+ (void)typeIntoField:(NSString*)text view:(UIView*)view error:(NSError**)error
{
	[self tapView:view error:error];
	CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0.5, false);
...
+ (void)typeIntoField:(NSString*)text element:(UIAccessibilityElement*)element error:(NSError**)error
{
    UIView *view = [UIAccessibilityElement viewContainingAccessibilityElement:element];
	[self typeIntoField:text view:view error:error];
}

+ (void)tapElementWithLabel:(NSString*)label  error:(NSError**)error
{
...
+ (KIFTestStepResult)tapElement:(UIAccessibilityElement *)element error:(NSError**)error
{
	KIFTestCondition(element, error, @"The element was nil");
	UIView *view = [UIAccessibilityElement viewContainingAccessibilityElement:element];
	KIFTestCondition(view, error, @"The view was nil");
	return [self tapView:view error:error];
}

+ (KIFTestStepResult)tapCharactersInString:(NSString *)string error:(NSError**)error
{
...
+ (KIFTestStepResult)tapView:(UIView *)view error:(NSError**)error
{
...
+ (KIFTestStepResult)tapButton:(NSString *)label error:(NSError**)error
{
	...
	if(buttonElement != nil) {
		...
		if (buttonView.alpha > 0) {
			...
			return [self tapView:buttonView error:error];
		}
	}
	return KIFTestStepResultSuccess;
}
